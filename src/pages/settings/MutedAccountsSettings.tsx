...
export const MutedAccountsSettings: React.FC = () => {
  ...
  const handleToggle = (path: 'muted.hideStories' | 'muted.hidePosts', value: boolean) => {
    tap();
    ...
    <div className="min-h-screen bg-black leather-overlay pb-20">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="leather-card rounded-xl p-4 stitched">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h3 className="text-white font-semibold mb-1">Masquer les stories</h3>
              <p className="text-leather-300 text-sm">Masquer les stories des comptes masqués</p>
            </div>
            <button
              onClick={() => handleToggle('muted.hideStories', !preferences.muted.hideStories)}
            ...
            >
              <div
                className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${
                  preferences.muted.hideStories ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        <div className="leather-card rounded-xl p-4 stitched">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h3 className="text-white font-semibold mb-1">Masquer les posts</h3>
              <p className="text-leather-300 text-sm">Masquer les posts des comptes masqués</p>
            </div>
            <button
              onClick={() => handleToggle('muted.hidePosts', !preferences.muted.hidePosts)}
              className={`relative w-14 h-8 rounded-full transition-colors ${
            ...
            >
              <div
                className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${
                  preferences.muted.hidePosts ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        {mutedAccounts.length > 0 && (
          <div className="leather-card rounded-xl p-4 stitched">
            <h3 className="text-white font-semibold mb-3">Comptes masqués ({mutedAccounts.length})</h3>
