    ...
    <div className="min-h-screen bg-black leather-overlay pb-20">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
            ...
            >
              <div
                className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${
                  preferences.storage.dataSaver ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        <div className="leather-card rounded-xl p-4 stitched">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h3 className="text-white font-semibold mb-1">Télécharger uniquement en WiFi</h3>
              <p className="text-leather-300 text-sm">Télécharger uniquement lorsque connecté au WiFi</p>
            </div>
            <button
              onClick={() => handleToggle('storage.downloadWifiOnly', !preferences.storage.downloadWifiOnly)}
              className={`relative w-14 h-8 rounded-full transition-colors ${
                preferences.storage.downloadWifiOnly ? 'bg-gold-500' : 'bg-leather-700'
              }`}
            >
              <div
                className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${
                  preferences.storage.downloadWifiOnly ? 'translate-x-6' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
